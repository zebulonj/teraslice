...
module.exports = function (context) {
    ...
    Promise.resolve(require('./storage/assets')(context))
        .then((assetStore) => {
            logger.warn(`node ${context.sysconfig._nodeName} is loading assets ${job.assets}`);
            return loadAssets(assetStore, job.assets)
        })
        .then((assetArray) => {
            if (process.env.preload) {
                logger.warn(`i should only be preloading the assets once ${exId}`)
                messaging.respond(respondingData, { message: 'assets:preloaded', meta: assetArray });
            } else {
                logger.warn(`node ${context.sysconfig._nodeName} is done loading assets for ${exId} , now reporting back`)
