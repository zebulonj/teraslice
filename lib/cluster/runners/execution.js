    ...
    function initialize(events, logger) {
        return new Promise(((resolve, reject) => {
            ...
            events.on('execution:assets_loaded', (ipcMessage) => {
                logger.warn(`the event emitter for ${process.env.assignment} has been called`, ipcMessage, gettingJob)
                if (ipcMessage.error) {
                    ...
                    gettingJobInterval = setInterval(() => {
                        if (!gettingJob) {
                            ...
                            logger.warn(`ex: ${exId}, type: ${process.env.assignment} on node ${context.sysconfig._nodeName} got asset event`)

                            Promise.resolve(_instantiateJob())
                                .then((jobInstance) => {
                                    events.removeAllListeners('execution:assets_loaded');
                                    logger.warn(`ex: ${exId}, type: ${process.env.assignment} on node ${context.sysconfig._nodeName} loaded executionConfig`)
                                    resolve(jobInstance);
                                })
                                .catch((err) => {
                                    events.removeAllListeners('execution:assets_loaded');
            ...
            Promise.resolve(_instantiateJob())
                .then((jobInstance) => {
                    logger.warn(`ex: ${exId}, type: ${process.env.assignment} on node ${context.sysconfig._nodeName} immediately instantiated executionConfig`)
                    events.removeAllListeners('execution:assets_loaded');
                    clearInterval(gettingJobInterval);
                    resolve(jobInstance);
                })
                .catch((err) => {
                    ...
                    if (!needAssets) {
                        ...
                        logger.warn(`ex: ${exId}, type: ${process.env.assignment} on node ${context.sysconfig._nodeName} is waiting`)
