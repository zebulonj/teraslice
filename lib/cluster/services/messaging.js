    ...
    if (config.clients.ipcClient) {
        process.on('message', (ipcMessage) => {
            ...
            if (realMsg) {
                ...
                if (realMsg === 'assets:loaded') logger.warn(`type: ${process.env.assignment}, ex: ${process.env.ex_id} on node: ${context.sysconfig._nodeName} got the asset loaded message`)
