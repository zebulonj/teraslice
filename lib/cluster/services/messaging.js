...
module.exports = function messaging(context, logger, childHookFn) {
    ...
    function sendToProcesses(msg) {
        ...
        if (msg.to === 'execution') {
            logger.warn(`node ${context.sysconfig._nodeName} got a execution message from asset loader`)
            _.each(childProcesses, (workerProcess) => {
                if (isExecutionProcess(workerProcess, msgExId)) {
                    ...
                    logger.warn('what am i sending to the execution processes', msg)
    ...
    if (config.clients.ipcClient) {
        process.on('message', (ipcMessage) => {
            ...
            if (realMsg) {
                ...
                if (realMsg === 'assets:loaded') logger.warn(`type: ${process.env.assignment}, ex: ${process.env.ex_id} on node: ${context.sysconfig._nodeName} got the asset loaded message`,realMsg, ipcMessage)
