...
module.exports = function messaging(context, logger, childHookFn) {
    ...
    // execution_controller may recieve a pause command before full up, so we need to buffer the
    // messages and emit them when its ready
    ...
    function handleIpcMessages() {
        ...
        const emitFn = (realMsg, ipcMessage) => process.emit(realMsg, ipcMessage);
